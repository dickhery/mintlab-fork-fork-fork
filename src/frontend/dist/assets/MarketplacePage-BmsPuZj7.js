...
import { R as Root, I as Item, c as createRovingFocusGroupScope, P as PriceDisplay, t as transferRegisteredNFT } from "./external-nft-transfer-DKb6WZ1m.js";
import { a as appPageUrl, l as listingSharePath, S as ShareLinkButton, b as listingShareUrl } from "./share-urls-DKbJaUql.js";
...
import { u as useDirection, S as Select, c as SelectTrigger, d as SelectValue, e as SelectContent, f as SelectItem } from "./select-CT7-_nRB.js";
import { u as usePageMeta } from "./use-page-meta-CcnDWWfp.js";
...
import { S as SlidersHorizontal } from "./sliders-horizontal-BFVzt0Tc.js";
import { S as Search } from "./search-BG7FIrIG.js";
import { G as Gavel } from "./gavel-m08JdVp4.js";
...
function MarketplacePage() {
  ...
  const { data: settlementStatuses = [] } = useQuery({
    ...
    refetchInterval: (query) => {
      ...
      return statuses.length > 0 ? 3e5 : false;
    }
  });
  ...
  async function findSellerListingForNFT(nft, sellerPrincipal, kind) {
    if (!actor) return null;
    const details = await actor.getActiveListingDetails();
    return details.find((detail) => {
      if (detail.nft.collectionId !== nft.collectionId || detail.nft.tokenId !== nft.tokenId) {
        return false;
      }
      if (kind === "fixed") {
        return detail.listing.__kind__ === "Fixed" && detail.listing.Fixed.seller.toString() === sellerPrincipal;
      }
      return detail.listing.__kind__ === "Auction" && detail.listing.Auction.seller.toString() === sellerPrincipal;
    }) ?? null;
  }
  function warmListingShareImage(listingId) {
    if (!actor) return;
    ...
    mutationFn: async ({ nft, price }) => {
      ...
      try {
        ...
        if (principal && isAmbiguousListingFailureError(error)) {
          ...
          if ((recovered == null ? void 0 : recovered.listing.__kind__) === "Fixed") {
    ...
    mutationFn: async ({
      ...
      try {
        ...
        if (principal && isAmbiguousListingFailureError(error)) {
          ...
          if ((recovered == null ? void 0 : recovered.listing.__kind__) === "Auction") {
